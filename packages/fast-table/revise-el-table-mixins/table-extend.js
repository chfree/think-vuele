import ElTable from '../table' // '../../../packages/table'
...
ElTableHeader.methods.toggleAllSelection = function(event) {
  ...
  if (this.$parent.useVirtual && this.$parent.bigDataCheckbox) {
    if (!fasle) {
      ...
      const flset = this.store.states.selection.filter(item => {
      ...
      if (flset.length < this.store.states.data.length) {
        ...
        this.store.states.selection = this.store.states.data.filter(item=> item) // 赋值全部数据
        const hoverRow2 = this.store.states.hoverRow
        // 然后改变states的hoverRow值
        this.store.states.hoverRow = -1
        // 异步赋值回去当前获取焦点的hoverRow，从而更新视图 （很重要）
        this.$parent.$nextTick(() => {
          this.store.states.hoverRow = hoverRow2
        })
